...
export async function POST(req: NextRequest) {
  ...
  let body: { prompt: string; gammaId: string; themeId?: string };
  try {
    body = await req.json();
  } catch {
  ...
  const { prompt, gammaId, themeId } = body;
  if (!prompt?.trim()) {
    return NextResponse.json({ error: "prompt is required." }, { status: 400 });
  }
  if (!gammaId?.trim()) {
    return NextResponse.json({ error: "gammaId is required." }, { status: 400 });
  }

  // ── Step 1: kick off generation from template ─────────────────────────────────
  let createData: { generationId?: string; error?: string };
  try {
    const createRes = await fetch(`${GAMMA_API}/generations/from-template`, {
      method: "POST",
      headers: {
    ...
    console.error("[gamma] create-from-template request failed:", err);
    return NextResponse.json(
    ...
    if (pollData.status === "completed") {
      ...
      if (pollData.exportUrl && process.env.BLOB_READ_WRITE_TOKEN) {
        try {
          ...
          pptxUrl = pollData.exportUrl;
        }
      }

      return NextResponse.json({
        gammaUrl,
        gammaId: resultGammaId,
        pptxUrl,
        title,
        generationId,
      });
